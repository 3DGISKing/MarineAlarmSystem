...
class GUI_EXPORT QgsRubberBand: public QgsMapCanvasItem
{
  public:
    ...
    */
	...
	//added by Forest Academy
	void setUsableFalg( bool flg );
	void addToCache( QgsGeometry* geom, QgsVectorLayer* layer);
	void addPoint(const QgsPoint& pt, QVector<QPointF>& contents );
	void addPoint(const QVector<QgsPoint>& source, QVector<QPointF>& target, QgsVectorLayer* layer, const long& sPos,const long& size );
	//end
  protected:
	...
	//added by Forest Academy
	void drawToImage( const QVector<QPointF>& contents );
  private:
    QBrush mBrush;
    QPen mPen;
